...
# let's select a total of 20 replicates for each seaweed (because we have 30 replicates only for sargassum and harpoon weeds
...
# Testing the initial size of prawns among experimental conditions --------
...
green_colour_space_lab_a <-
  ...
  geom_point(
    ...
    fill = hex(LABdata_green, fix = TRUE), alpha = 0.8, size = 6) +
  stat_ellipse(
...
green_colour_space_lab_c <-
  ...
  theme(
    ...
    legend.position.inside = c(0.375, 0.85),
...
red_colour_space_lab_a <-
  ...
  geom_point(
    data = colour_change_data  |> 
           filter(type == "r", day %in% c(0, 30)),
    aes(x = rnl_x_mean, y = rnl_y_mean, shape = seaweed),
    fill = hex(LABdata_red, fix = TRUE), alpha = 0.8, size = 6) +
  stat_ellipse(
    data = seaweed_data_edited  |> 
...
for (i in 1:dim(colour_change_data_g_hw)[1])
{
  ...
  {
    jnd_g_hw[j] <-
      sqrt(((colour_change_data_g_hw$rnl_x_mean[i] - seaweed_data_hw$rnl_x_mean[j])^2) +
        ((colour_change_data_g_hw$rnl_y_mean[i] - seaweed_data_hw$rnl_y_mean[j])^2))
  }
...
for (i in 1:dim(colour_change_data_g_sm)[1])
{
  ...
  {
    jnd_g_sm[j] <-
      sqrt(((colour_change_data_g_sm$rnl_x_mean[i] - seaweed_data_sm$rnl_x_mean[j])^2) +
        ((colour_change_data_g_sm$rnl_y_mean[i] - seaweed_data_sm$rnl_y_mean[j])^2))
  }
...
for (i in 1:dim(colour_change_data_g_d)[1])
{
  ...
  {
    jnd_g_d[j] <-
      sqrt(((colour_change_data_g_d$rnl_x_mean[i] - seaweed_data_d$rnl_x_mean[j])^2) +
        ((colour_change_data_g_d$rnl_y_mean[i] - seaweed_data_d$rnl_y_mean[j])^2))
  }
...
for (i in 1:dim(colour_change_data_r_hw)[1])
{
  ...
  {
    jnd_r_hw[j] <-
      sqrt(((colour_change_data_r_hw$rnl_x_mean[i] - seaweed_data_hw$rnl_x_mean[j])^2) +
        ((colour_change_data_r_hw$rnl_y_mean[i] - seaweed_data_hw$rnl_y_mean[j])^2))
  }
...
for (i in 1:dim(colour_change_data_r_sm)[1])
{
  ...
  {
    jnd_r_sm[j] <-
      sqrt(((colour_change_data_r_sm$rnl_x_mean[i] - seaweed_data_sm$rnl_x_mean[j])^2) +
        ((colour_change_data_r_sm$rnl_y_mean[i] - seaweed_data_sm$rnl_y_mean[j])^2))
  }
...
for (i in 1:dim(colour_change_data_r_sl)[1])
{
  ...
  {
    jnd_r_sl[j] <-
      sqrt(((colour_change_data_r_sl$rnl_x_mean[i] - seaweed_data_sl$rnl_x_mean[j])^2) +
        ((colour_change_data_r_sl$rnl_y_mean[i] - seaweed_data_sl$rnl_y_mean[j])^2))
  }
...
# joining the panel of the RNL colour space and the JND graphic of green and red prawns

## Green prawns -------------------------------------------------------------

graph_colour_change_green <-
  green_colour_space_lab_c +
  graph_colour_jnd_green_c +
  plot_layout(ncol = 1) +
  plot_annotation(tag_levels = "a", tag_prefix = "(", tag_suffix = ")")

graph_colour_change_green[[1]] <-
  graph_colour_change_green[[1]] +
  theme(plot.margin = margin(0, 0.5, 0, 0.5, "cm"))

graph_colour_change_green[[2]] <-
  graph_colour_change_green[[2]] +
  theme(plot.margin = margin(0, 0.5, 0, 0.5, "cm"))
...
graph_colour_change_red <-
  red_colour_space_lab_c +
  graph_colour_jnd_red_c +
  plot_layout(ncol = 1) +
  plot_annotation(tag_levels = "a", tag_prefix = "(", tag_suffix = ")")

graph_colour_change_red[[1]] <-
  graph_colour_change_red[[1]] +
  theme(plot.margin = margin(0, 0.5, 0, 0.5, "cm"))

graph_colour_change_red[[2]] <-
  graph_colour_change_red[[2]] +
  theme(plot.margin = margin(0, 0.5, 0, 0.5, "cm"))
...
# spliting the dataset between training and testing samples with a 60% cut-off
