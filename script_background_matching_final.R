...
graph_jnd_luminance_seaweeds <-
  ggplot(
    background_matching_data_final,
    aes(x = jnd_luminance, fill = seaweed, alpha = seaweed)) +
  geom_histogram(binwidth = 1, alpha = 0.5) +
  scale_x_continuous(
    name = "Luminance JND",
  ...
  geom_vline(
    data = mean_jnd_sargassum,
    aes(xintercept = jnd_luminance_mean), colour = "#663300",
    linewidth = 1, linetype = "dashed") +
  geom_vline(
    data = mean_jnd_harpoon,
    aes(xintercept = jnd_luminance_mean), colour = "#FF9999",
    linewidth = 1, linetype = "dashed") +
  theme_bw() +
  theme(
    plot.margin = margin(0.2, 0.2, 0.2, 0.2, "cm"),
